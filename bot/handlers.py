...
async def start(update: tg.Update, context: tg_ext.ContextTypes.DEFAULT_TYPE) -> int:
    keyboard = [['Новое слово', 'Старое слово']]
...
async def new_word(update: tg.Update, context: tg_ext.ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text('Введите слово на английском')
    print('new_word сработало')
    return ENG_WORD


async def new_word_eng(update: tg.Update, context: tg_ext.ContextTypes.DEFAULT_TYPE) -> int:
    eng_word = update.message.text
...
async def new_word_ru(update: tg.Update, context: tg_ext.ContextTypes.DEFAULT_TYPE) -> int:
    ru_word = update.message.text

    print(context.user_data['eng_word'], ru_word)
...
def setup_handlers(application: tg_ext.Application) -> None:
    ...
    conv_handler = tg_ext.ConversationHandler(
        ...
        states={
            START: [
                tg_ext.MessageHandler(tg_ext.filters.Regex('^(Новое слово|Старое слово)$'), new_word)
            ],
            ENG_WORD: [
